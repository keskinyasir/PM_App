...
def delete_project(pid):
    with get_connection() as conn:
        ...
        conn.commit()
    st.success(f"Project {pid} and its tasks deleted")
        ...
        if menu == 'Dashboard':
            ...
            with st.expander('➕ Add New Project'):
                ...
                if st.button('Create Project'):
                    if name and end >= start:
                        ...
                        st.error("Please provide a project name and ensure End Date is after Start Date")
            ...
            if not dfp.empty:
                ...
                sel = st.selectbox(
                    ...
                    format_func=lambda x: f"{x} - {dfp[dfp['id']==x]['name'].iloc[0]}"
                )
            ...
            with st.expander('➕ Add New Task'):
                ...
                if not dproj.empty:
                    pid = st.selectbox(
                        ...
                        format_func=lambda x: f"{x} - {dproj[dproj['id']==x]['name'].iloc[0]}"
                    )
                    ...
                    if st.button('Add Task'):
                        if title and due:
                            ...
                            st.error("Please provide task title and due date")
                else:
